...
class SaleOrderLineTalla(models.Model):
    _name = 'sale.order.line.talla'
    _description = "Tabla de tallas"

    order_id = fields.Many2one('sale.order',string="Orden de Venta",ondelete="cascade")
    order_line_id = fields.Many2one('sale.order.line',string="Linea de Orden de Venta")
    ...
    def compute_campo_product_id(self):
        for rec in self:
            ...
            if rec.order_line_id:
                rec.product_template_id = rec.order_line_id.product_template_id or False 
                rec.product_id = rec.order_line_id.product_id or False """
    ...
    def compute_campo_display_type(self):
        for rec in self:
            rec.display_type = ''
            if rec.order_line_id:
                rec.display_type = rec.order_line_id.display_type or ''
